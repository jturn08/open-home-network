...
- [open-home-network](#open-home-network)
  - [Overview](#overview)
    - [Features](#features)
    - [Software \& Hardware overview](#software--hardware-overview)
  ...
  - [Software \& Hardware Details](#software--hardware-details)
    - [OpenWrt](#openwrt)
...
The Essential setup is for a home with all client devices connected wirelessly with no wired ethernet network cabling required. This is the simplest hardware and software setup. 

Functionality supported
- < 500 Mbps internet connection
- 1 or more Wi-Fi access points (wireless mesh connection)
...
- Wi-Fi Access Point(s): **OpenWrt** compatible Wi-Fi router

#### Setup guide  
1. [Installing OpenWrt on a Wi-Fi router](Installing-OpenWrt-on-a-Wi-Fi-router.md)  
2. [Configuring a Wi-Fi network in OpenWrt](Configuring-OpenWrt-with-essential-Wi-Fi-network.md)  

Optional:  

3. [Configuring a secondary OpenWrt device with Wi-Fi 802.11s mesh networking for improved connection speed & reliability](Configuring-secondary-OpenWrt-Wifi.md)  
4. Configuring OpenWrt with additional Guest Wi-Fi network to improve security - coming soon
5. Configuring Wireguard VPN for secure remote access - coming soon

### Deluxe setup: Wi-Fi and wired network
The Deluxe setup is for a home with client devices connecting both wirelessly and with wired ethernet, and requires wired ethernet network cabling between Wi-Fi access points.  
...
#### Setup guide
In addition to following the guides in the essential setup, the below guides for deluxe setup supports both Wi-Fi and wired ethernet network.  

1. [Configuring a secondary OpenWrt device with wired ethernet cable for improved connection speed & reliability](https://openwrt.org/docs/guide-user/network/wifi/dumbap)
...
The Advanced setup uses the Deluxe setup as a base and uses more powerful dedicated router hardware for faster speeds, advanced routing & firewall functionality, and is suitable for a home with multiple client devices connecting wirelessly and with wired ethernet network, and requires Cat5e or better wired ethernet network cabling. 
...
- Wi-Fi Access Point(s): **OpenWrt** compatible Wi-Fi router
- Wired networks: Managed wired ethernet network switch connected to Wi-Fi router and client devices (PCs)riuyter 
- Ultra-fast (5 GB per second) point-to-point wired network 
...
#### Hardware choices
OpenWrt runs on some Wi-Fi routers. Below are some devices under $100 with good OpenWrt support as of 2023:
...
- You'll need a PC with at least 2 wired network ports (1 for WAN and 1 for LAN) or a PCIe 2.0 x4 expansion slot to add a dual-port network card.
- Low power consumption PC is preferred. The PC will be running 24x7, so electricity costs add up as each Watt of electricity will cost ~$1 to run 24/7 for a year.
